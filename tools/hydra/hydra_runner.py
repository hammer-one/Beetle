...
class HydraRunner:
        ...
        try:
            ...
            for line in iter(proc.stdout.readline, ''):
                            ...
                            if any(k in lower for k in ["login", "password", "found", "host", "successful", "valid", "[+]", "attempt"]):
                                ...
                                # Dividir en hasta 4 partes
                                max_line_len = 21  # aprox lo que cabe bien en OLED
                                ...
                                while remaining and len(parts) < 4:
                                    if len(remaining) <= max_line_len:
                                        ...
                                        break
                                    # Buscar mejor punto de corte (espacio)
                                    split_point = remaining[:max_line_len].rfind(' ')
                                    if split_point > 8:  # evita cortar palabras muy cortas
    ...
    def _is_enter_pressed(self): 
        return read_buttons().get("enter", False)
